...
def replace_substring(sentence, start, end, new_substring):
    if 0 <= start <= end < len(sentence):
        return sentence[:start] + new_substring + sentence[end:]
    return sentence
...
def main():
    ...
    in_path = "Code/electronics_reviews_2004/"
    ...
    for file in files:
        ...
        if len(name_split) == 1:
            ...
            name = name_split[0]
        out_path = "Code/data_out/electronics_reviews_2004/" + name + "/raw_data_" + name + "_2004.txt"
        ...
        print("Implicit: " + str(implicit) + ' and Percentage: ' + str(implicit/(implicit + conflicts + other + count)))
        print("Conflicts: " + str(conflicts) + ' and Percentage: ' + str(conflicts/(implicit + conflicts + other + count)))
        print("Other: " + str(other) + ' and Percentage: ' + str(other/(implicit + conflicts + other + count)))
        
        print(f'Positive sentiment: {aspect_polarity.count(1)} {(100 * aspect_polarity.count(1) / len(aspect_polarity))}')
        print(f'Negative sentiment: {aspect_polarity.count(-1)} {(100 * aspect_polarity.count(-1) / len(aspect_polarity))}')
        print(f'Neutral sentiment: {aspect_polarity.count(0)} {(100 * aspect_polarity.count(0) / len(aspect_polarity))}')
