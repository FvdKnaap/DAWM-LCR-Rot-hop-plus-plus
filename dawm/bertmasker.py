...
def concatenate_tensors(representation: torch.Tensor, descriptor: torch.Tensor, domain_list: torch.Tensor,att_masks):
    ...
    for idx, value in enumerate(domain_list):
        
        # Get the corresponding embedding representation [MAX_LENGTH x hidden_dim]
        slice1 = representation[idx]
        
        # Get the descriptor of the domain belonging to this sample [descriptor_dimensions]
        slice2= descriptor[value]
        
        # Repeat for every token in the sentence to concatenate both tensors
        slice3 = slice2.repeat(MAX_LENGTH,1)
        
        concatenated_tensor = torch.cat((slice1, slice3), dim=-1)
        concatenated_tensor2 = torch.cat((slice1, ave_descr2), dim=-1)
        concatenated_tensor3 = torch.stack((concatenated_tensor,concatenated_tensor2),dim=-2)
        concatenated_tensors.append(concatenated_tensor3)
...
def mix_domain_descriptors(representation: torch.Tensor, descriptor: torch.Tensor):
    ...
    representation = representation.unsqueeze(1).expand(representation.size(0),num_domains,hidden_dim)
    
    # add dimension to the domain descriptor to match cls representation [batch_size x num_domains x descriptor_dimension]
    descriptor = descriptor.unsqueeze(0).expand(representation.size(0),num_domains,descriptor_dimension)
...
class PrivatePart(nn.Module):
    ...
    def forward(self, hidden_embeddings: torch.Tensor, input_embedding: torch.Tensor, mask_embedding: torch.Tensor, segments_tensor: torch.Tensor,domain_list,z):
       ...
       # z = concatenate_tensors2(representation=hidden_embeddings,descriptor=self.descriptors,domain_list=domain_list)
        ...
        #domain_descr_mix = torch.matmul(similarity_sm, self.descriptors).unsqueeze(1).expand(hidden_embeddings.size(0),MAX_LENGTH,descriptor_dimension)
        ...
        #private_rep2 = torch.sum(embedded_inputs * segments_tensor.unsqueeze(-1) * (1-zeros).unsqueeze(-1),dim=1) / torch.sum(segments_tensor * (1-zeros),dim=1).unsqueeze(-1)
        ...
        #private_rep = torch.sum(hidden_states[:,:,:,:], dim=0)
        ...
        y_pred = self.dcp(private_rep2)

        
        self.i+=1
        return y_pred,private_rep,embedded_inputs,P[torch.arange(P.size(0)),:, 0]
...
class SharedPart(nn.Module):
    ...
    def forward(self, hidden_embeddings: torch.Tensor, input_embedding: torch.Tensor, mask_embedding: torch.Tensor, segments_tensor: torch.Tensor, domain_list,z):
       ...
       # shared_rep = torch.sum(hidden_states[:,:,:,:], dim=0)
        ...
        #print(torch.sum((P[torch.arange(P.size(0)),:, 0]) * segments_tensor * (1-zeros),dim=1) / torch.sum(segments_tensor * (1-zeros),dim=1) )
        condition = sum == 0
        private_rep2 = torch.sum(embedded_inputs * segments_tensor.unsqueeze(-1) * (1-zeros).unsqueeze(-1),dim=1) / (sum + condition.float())
        ...
        return y_pred,shared_rep,y_pred2,private_rep,mask_perc#embedded_inputs,P[torch.arange(P.size(0)),:, 0]
...
class BERTMasker(nn.Module):
    ...
    def forward(self, hidden_embeddings: torch.Tensor, input_embedding: torch.Tensor, mask_embedding: torch.Tensor, segments_tensor: torch.Tensor, domain_list: list):
        """
        :param hidden_embeddings: [batch_size x MAX_LENGTH x hidden_dim] containing all token embeddings
        :param input_embedding: [batch_size x MAX_LENGTH x hidden_dim] containing the input embeddings used by BERT to egenrate contextualized embeddings 
        :param mask_embedding: [hidden_dim] containing the input embedding of the [MASK] token
        ...
        #sentiment_cls = self.softmax(sentiment_cls)
        total_cls = torch.cat((shared_cls,private_cls),dim=-1)
        
        sentiment_pred = self.sentiment_classifier(total_cls)
        
        #return sentiment_pred
        return shared_output,private_output,sentiment_pred,shared_output,private_output,sentiment_pred,mask_perc#p_embedded_inputs,s_embedded_inputs,p_P,s_P
