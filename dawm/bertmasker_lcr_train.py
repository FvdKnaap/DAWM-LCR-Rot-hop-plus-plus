...
def main():
    ...
    for epoch in range(epochss):
        ...
        '''
            ...
            #for name, param in shared_part.named_parameters():
             #   if 'bert' not in name:
             #       param.requires_grad = False
            
            #for name, param in private_part.named_parameters():
            #    if 'bert' not in name:
            #        param.requires_grad = False
        '''
        ...
        with tqdm(total=len(data_loader), desc=f"Epoch {epoch+1}/{epochss}", unit="batch") as pbar:
            for batch_idx,(hidden_embeddings,_,segments_tensor,polarities,domain,target_indices,_,input_embedding,domain_list,_,_,_,_,_,_) in enumerate(data_loader):
                ...
                #train_acc_now = (torch.argmax(nn.functional.softmax(sentiment_pred,dim=-1),dim=1) == torch.argmax(polarities,dim=1)).type(torch.int)
                #train_acc += torch.sum((1-domain_list) * train_acc_now)
                #total += (1-domain_list).sum()
    	        

                #train_acc += torchmetrics.functional.accuracy(torch.argmax(nn.functional.softmax(sentiment_pred,dim=-1),dim=1), torch.argmax(polarities,dim=1),task='multiclass',num_classes=num_polarities,average='none')
    ...
    for current_domain in ['Laptop','Restaurant','Book']:
        ...
        with torch.no_grad():
            for batch_idx,(hidden_embeddings,_,segments_tensor,polarities,domain,target_indices,_,input_embedding,domain_list,_,_,_,_,_,_) in enumerate(test_dataloader):
                ...
                #accuracy += torchmetrics.functional.accuracy(pred, polarities,task='multiclass',num_classes=num_polarities,average = 'none')
                #f1 += torchmetrics.functional.f1_score(pred,polarities,task='multiclass',num_classes=num_polarities,average='none')
